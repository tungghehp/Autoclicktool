...
layout = [
    ...
    [sg.Column([
        ...
        [sg.Text("Thời gian (giây):", font=font_text, size=(12,1), justification='right', text_color="white"),
         sg.InputText("0.5", key='-TIMECLICK-', size=(8,1), font=font_text, text_color="#00ffcc", background_color="#1e1e1e", justification='center')],
...
while True:
    ...
    if keyboard.is_pressed('f6'):
        try:
            timeclick = float(values['-TIMECLICK-'])
        except:
            timeclick = 0.5  # fallback nếu nhập sai
        toggle_status()
        time.sleep(0.5)
        if status:
